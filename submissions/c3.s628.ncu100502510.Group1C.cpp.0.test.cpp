...
int main()
{
    ...
    for(unsigned int i=0;i<input.length();i++)
    {
        ...
        else if(((input[i]>=97&&input[i]<=122)||(input[i]>=65&&input[i]<=90))&&((input[i+1]>=48&&input[i+1]<=57)||(input[i-1]>=48&&input[i-1]<=57)))
        {
            cout<<"invalid input";
            system("pause");
            return 0;
        }
        else if((input[i]>=48&&input[i]<=57)&&(((input[i-1]>=97&&input[i-1]<=122)||(input[i-1]>=65&&input[i-1]<=90))||((input[i+1]>=97&&input[i+1]<=122)||(input[i+1]>=65&&input[i+1]<=90))))
        {
            cout<<"invalid input";
            system("pause");
            return 0;
