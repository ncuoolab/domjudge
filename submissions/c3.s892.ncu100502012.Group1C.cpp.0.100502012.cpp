	...
	while( true ) {
		...
		while( input != "" ) {
			...
			if( input_char[0] =='(' ) {
				...
				bool jump =false;
				while( ( input_char[char_count] >='a' && input_char[char_count] <='z') || ( input_char[char_count] >='A' && input_char[char_count] <='Z') || ( input_char[char_count] =='_') || ( input_char[char_count] =='(' ) ) {
					if ( input_char[char_count] =='(') {
						string temp(input.begin(), input.begin() + char_count );
						tokens[token_count].ter ="id";
						tokens[token_count].var = temp;
						token_count++;
						last_token.ter ="id";
						last_token.var = temp;
						string temp2(input.begin() + char_count , input.end() );
						input.assign(temp2);
						input_char = input.c_str();
