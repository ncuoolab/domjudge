...
#define TYPE_PROC (1)
// ==== TODO 1/5 Token Defs. Here, EXCEPT "TYPE_PROC" (root) ====
#define TYPE_DCL (2)
...
void runScanner(){
	...
	while(scanf("%c", &c) != EOF){
		...
		if( c=='"' ){
			...
			if(isStrNonEmpty && currentType!=TYPE_STRING){
				addToken(currentType, tokenPtr->c_str());
				tokenPtr->clear();
			}
			...
			if(c=='s')currentType = TYPE_STRDCL;
...
MyNode* checkGrammarRecur(MyNode *begin, int typeRequired){
	...
	//TODO 3/5 modify grammar rules here
	/* [Hints]
...
int main(){
	...
	if(checkGrammar(head->nextNode, TYPE_PROC)){
		//TODO 4/5 when the parser process succeeds
		...
		while(iterNode!=NULL){
			if(iterNode->type==TYPE_STRDCL){
				...
				if(iterNode->content[0]!='\0')printf("string %s\n", iterNode->content);
		...
		//TODO 5/5 when the parser process fails
		printf("valid input\n");
	}
	//system("pause");
	return 0;
