...
string Match_id(string s , int id_index){
	...
	for(unsigned int i = 0 ; i <= id_index ; i++){
		if(s[i] >= 65 && s[i]<= 90) {
...
int main(){
	...
	int id_index = 0, Lparenthesis_index =0 , Para_index = 0, Rparenthesis_index = 0;
	getline(cin , input);
	...
	if(id_index == 0 || Lparenthesis_index == 0 || Para_index == 0 ||  Rparenthesis_index == 0){
		answer_valid = false;
	}
	else{
