...
void ScanLastName() {
    ...
    while(inputIndex != input.length() && (input.at(inputIndex) == 'a' || (input.at(inputIndex) >= 'c' && input.at(inputIndex) <= 'z'))) {
        string strValue(1,input.at(inputIndex++));
        outputValue += strValue;
    }
    outputValue += ";";
...
bool Scanner() {
    ...
    if(input.at(inputIndex) >= '0' && input.at(inputIndex) <= '9')
        ...
        if(input.at(inputIndex) == 'l' && (inputIndex != input.length() && input.at(inputIndex+1) >= '0' && input.at(inputIndex+1) >= '9'))
            ScanLevel();
